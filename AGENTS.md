...
Все тексты, которые видит пользователь, должны быть **на английском языке**:
...
Комментарии, описывающие UI и бизнес-логику, предпочтительно писать на английском.
...
- Если атом влияет на UI, добавлять короткий комментарий на английском:
...
- При добавлении новых IPC-каналов:
  ...
  - кратко задокументировать (на английском) назначение канала.
...
- Классы `PascalCase`, название файлов тоже в  `PascalCase`
...
- `type` использовать для:
  ...
  - utility-типов.
- После изменения файлов делять typecheck и исправлять ошибки typescript
...
- Использовать существующий стиль проекта и форматтер (eslint и т.п.).
- Не изменять форматирование больших блоков кода без необходимости (чтобы не засорять диффы).
- После изменнеия прогонять eslint и исправлять
...
- Ошибки:
  ...
  - пользователю показывать сообщение без технических подробностей.
